...
- `skills`：slash-callable Skill 入口和复用能力。
...
MVP 不使用 Claude Code 的 `commands/` 目录。用户可显式调用的插件入口全部来自 `skills/<skill-name>/SKILL.md` 暴露的 slash-callable Skill，并采用插件命名空间。MVP 推荐 Skill 入口：
...
上述入口既可以由用户显式 slash 调用，也可以由 `workflow` 根据 `state.json`、`current-task.json` 和阶段状态在会话中引导使用。外部调用方式不构成安全边界；安全边界由 Skill 内部人工确认闸口、确定性脚本状态校验和 Hook 校验共同承担。

workflow 是主编排入口：负责读取当前任务、判断任务阶段、选择下一步合法动作，并在 Claude Code 会话中引导使用对应 Agent/Skill；Agent 负责专业视角，Skill 负责具体工作方法、产物生成和内部闸口；脚本/Hook 负责确定性状态读写与硬校验。

workflow 采用 taskType-based resolver 机制，避免把 feature 流程写死到统一入口，同时避免为每种任务类型提前新增一层 workflow Skill：
...
/scc-dev-sphere:workflow
  ...
  -> 识别 taskType
  -> 调用 scripts/devsphere-workflow.js
  -> 委派给对应 taskType resolver
```

MVP 只实现 feature resolver：

- `skills/workflow/SKILL.md`：统一用户入口，负责读取 current task、调用确定性 workflow 脚本、解释 `nextAction`，并在 Claude Code 会话中引导使用对应 Agent/Skill 或暂停等待人工确认。
- `scripts/devsphere-workflow.js`：通用确定性工具，负责读取 current task、加载 state、校验 taskPath、调用 taskType 对应 resolver。
- `scripts/workflows/feature-workflow.js`：deterministic next-action resolver，根据 `state.status`、`stages`、`workflowMode`、`humanGateStages`、review matrix、approval 计算下一步合法动作描述。

resolver 脚本只输出结构化 next action 和准入条件，不生成设计正文、不调用 Agent、不修改产物、不替代 Skill 内部人工确认。`workflow` Skill 根据 next action 在 Claude Code 会话中引导使用对应 Agent/Skill，执行结果由 Hook/scripts 做确定性校验和状态同步。

MVP 采用“产物 + 状态驱动”的 workflow/Skill 交互模型。通用 `/workflow` 读取当前任务、识别 `taskType` 并调用对应 resolver；feature 流程由 `scripts/workflows/feature-workflow.js` 结合 `state.json`、`stages`、预期产物、review matrix、人工确认记录和批准记录计算下一步 `nextAction`。Skill 的核心职责是生成或更新约定产物；Skill 可以在会话中表达 `success`、`needs_input`、`failed` 等执行感知，但 workflow 不把该表达作为唯一机器依据。scripts/hooks 不解释业务流程意图，只做确定性校验、登记和状态同步。

resolver 输出必须遵循最小稳定 `nextAction` schema。该 schema 是 `workflow` Skill、taskType resolver、`status` Skill 和后续 taskType resolver 的接口契约；它只是确定性下一步动作描述，不是执行引擎或 DSL。
...
| kind | 含义 |
| --- | --- |
| `run_skill` | workflow 在 Claude Code 会话中引导使用对应 Agent/Skill。 |
| `human_confirm` | workflow 暂停并等待人工确认。 |
| `show_status` | 只展示状态、待办或下一步建议，不推进流程。 |
| `blocked` | 展示阻塞原因和恢复建议，不自动推进。 |
| `completed` | 展示完成态，不继续推进。 |

`human_confirm` 不引入统一 confirmation registry，也不定义统一确认 payload。它只表示 workflow 需要暂停等待人工确认；确认提示放入 `pause.prompt`，确认内容由具体场景生成，确认结果由对应 Skill/script 写入既有过程件，hooks/scripts 校验落盘结果。
...
workflow 负责展示确认项并收集用户选择；Skill/script 负责写入确认结果；hooks/scripts 只校验对应过程件是否存在有效记录。Hook 不得自动替用户选择确认结果。

单 Agent 或多 Agent 参与都使用 `agents[]` 表达期望职责视角。`agents[]` 是角色清单，不是 Skill 参数，也不是脚本级调度计划。
...
```json
{
  ...
  "expectedArtifacts": [
    "reviews/solution-design/sa-review.md",
    "reviews/solution-design/mde-review.md",
    "reviews/solution-design/tse-review.md",
    "reviews/review-matrix.json"
  ],
  "pause": null
}
...
```json
{
  ...
  "args": {},
  "agents": [],
  "reason": "businessDesign passed AI review and requires human approval",
  "requiredArtifacts": [
    "artifacts/business-design.md",
  ...
  "pause": {
    "type": "stage_approval",
    "prompt": "请确认 businessDesign 是否通过阶段人工评审。"
  }
}
```
...
- `feature-review` 不接收 `reviewer` 参数；评审 Agent 由 workflow / feature resolver 通过 `agents[]` 表达为期望参与角色，Agent 职责视角来自 Agent 自身定义。
- `nextAction` 不包含 `afterCompletion`。后续动作必须重新调用 resolver 计算。
- `nextAction` 不包含 `stateEffects`。允许/禁止的状态变化由状态机、scripts 和 Hook 校验。
- `nextAction` 不包含 `guards`。guard 规则是 resolver/hook 内部实现，必要时通过 `reason` 给出人类可读说明。
- `nextAction` 不包含 `onFailure`。失败处理回到 resolver，根据当前状态和错误记录重新计算。
- `kind=human_confirm` 已表达人工暂停，不再使用 `requiresHumanInput` 或顶层 `humanPrompt`。
- resolver 不引入复杂 DSL、条件表达式或自建 runtime。
...
| `assessed` | 进入设计阶段，推进业务设计 | `run_skill feature-design-business`，`agents=[sa]` | 需求不完整时澄清 | `business-design.md` |
| `designing` + `businessDesign` 未可用 | 生成/修订业务设计；完成后 SE 正式评审 | 先 `feature-design-business agents=[sa]`，后 `feature-review args.target=business-design agents=[se]` | strict/humanGate 命中时人工确认 | business artifact、SE review |
| `businessDesign` 可用，`solutionDesign` 未可用 | 生成/修订方案设计；完成后 SA/MDE/TSE 正式评审 | 先 `feature-design-solution agents=[se]`，后 `feature-review args.target=solution-design agents=[sa,mde,tse]` | strict/humanGate 命中时人工确认 | solution artifact、reviews |
| `solutionDesign` 可用，`implementationDesign` 未可用 | 生成/修订实现设计；完成后 SE/DEV/TSE 正式评审 | 先 `feature-design-implementation agents=[mde]`，后 `feature-review args.target=implementation-design agents=[se,dev,tse]` | strict/humanGate 命中时人工确认 | implementation artifact、reviews |
| `solutionDesign` 可用，`testDesign` 未可用 | 生成/修订测试设计；完成后 SA/SE/MDE 正式评审 | 先 `feature-design-test agents=[tse]`，后 `feature-review args.target=test-design agents=[sa,se,mde]` | strict/humanGate 命中时人工确认 | test artifact、reviews |
| 四个阶段都可用，但无 `integrated-design.md` 或集成评审未通过 | 生成/刷新 integrated design；执行集成一致性评审 | `feature-design` 的 integrated 生成动作；后 `feature-review args.target=integrated-design agents=[sa,se,mde,tse]` | blocking/advisory/risk/assumption 待处理时暂停 | integrated design、integrated review |
...
| `approved_for_implementation` | 生成开发执行计划 | `run_skill feature-plan-implementation agents=[dev]` | 高风险/strict 时等待计划批准 | implementation plan |
| `implementation_planned` | 发起代码落地 | `run_skill feature-implement agents=[dev]` | 首次修改代码前动作级确认 | implementation log、代码变更 |
| `implementing` | 继续实现、修复、补测试；完成后进入验证闸口 | `run_skill feature-implement agents=[dev]` | 范围越界时暂停并回到计划/设计 | code changes、tests |
...
- 命中 CIE 风险：review matrix 追加 CIE，相关 review 的 `agents[]` 增加 `cie`。
- 代码实现后发现明显偏离开发执行计划：生成偏差提示，写入 implementation log，并要求人工确认后再继续。

MVP 中 workflow 每次只推进一个最小 nextAction，不跨多个高风险阶段。`implementationDesign` 和 `testDesign` 在依赖关系上都可在 `solutionDesign` 达到当前模式要求后开始，但 MVP `/workflow` 按顺序推进，先 implementationDesign，再 testDesign。该顺序是执行策略，不表示 `testDesign` 在设计逻辑上依赖 `implementationDesign`。
...
`status` MVP 不引入独立 taskType resolver。实现规则：
...
`task-list` 和 `task-switch` 能力保留，但不作为 MVP 独立 Skill 入口暴露，由 `workflow` 或 `status` 通过参数承担，例如：
...
后续扩展保留 task type 前缀命名原则，例如 bugfix、refactor、performance 等流程，但不在 MVP 技术方案中列具体 Skill 入口清单，避免被误读为第一版交付入口。
...
阶段状态只记录稳定边界，不记录 `drafting`、`ai_reviewing` 这类 Skill 执行中的瞬时状态。`ai_review_passed` 在 `auto-design` 和 `collaborative-design` 中可以作为后续 AI 设计阶段的输入，但不能作为代码落地依据。阶段级 `human_approved` 是 `strict-human-loop` 阶段推进的硬依据；代码落地依据是最终批准后的 `approved_for_implementation`。
...
- 阶段状态只由对应 Skill 或确定性 Hook/脚本更新，Agent 不直接在正文中自由声明状态。
...
非正常退出默认不写阶段 `blocked`。Skill 执行失败、缺少必要输入、需要人工补充或需要决策时，相关阶段保持或回到 `drafted`，并把问题写入 review 明细或 `decisions/*-decisions.md`。只有任务无法在本插件流程内继续推进，且不能通过普通人工补充、人工决策或记录假设解决时，才写入 `state.status=blocked`。
...
插件 Hook 配置统一放在 `hooks/hooks.json`。MVP 不用 Hook 做通用命令安全、危险命令防护或代码修改范围硬阻断；这些通用安全边界交由 Claude Code 自身的沙箱、权限和工具确认机制承担。直接 slash 调用高风险 skill 的流程准入检查必须覆盖 `UserPromptExpansion`，因为用户直接输入 `/plugin:skill` 不一定经过匹配 `Skill` 工具的 `PreToolUse`。`PostToolUse` 只用于登记、校验结果、状态同步、实现后 diff 摘要、偏差提示或补救，不能作为阻断安全边界。
...
| `PreToolUse` / `PermissionRequest` | MVP 不用于代码修改范围或危险命令防护；仅在需要时作为流程状态推进前的辅助校验 | 否，除非用于流程状态推进前置校验 | 不实现通用命令安全，不静态判断 Bash 最终文件影响，不做代码范围硬阻断 |
| `PostToolUse` | artifact/review/approval/evidence/decision 生成后的登记、索引、状态同步、实现后 diff 摘要、偏差提示和一致性复核 | 否 | 不作为阻断安全边界，不补救已经发生的未授权写入 |
...
- 未绑定 active task 时推进 scc-dev-sphere 需求开发流程状态。
- 任务未进入 `implementation_planned` 或 `implementing` 时启动 `feature-implement`。
- 设计评审存在未关闭阻塞项时生成实现批准。
- 开发执行计划缺失时进入代码落地。

MVP 不阻断通用文件编辑、Bash 命令或代码修改范围。代码范围偏差在实现后通过 diff summary 提示并记录。
...
- `advisory` 未完成人工确认。
- 低风险但存在不确定性的实现建议。
- 实现后 diff summary 与开发执行计划存在明显范围偏差。
...
- Skill 成功生成阶段 artifact 后，同步阶段状态为 `drafted`。
...
- Skill 执行失败、缺少必要输入、需要人工补充或需要决策时，阶段状态保持或回到 `drafted`，并记录问题；只有任务无法通过普通人工补充、人工决策或记录假设继续推进时，才同步任务级 `state.status=blocked`。
...
Skill 是具体工作能力单元，默认由 workflow 根据持久化状态确定下一步应使用的 Agent/Skill，并在 Claude Code 会话中引导执行；也可以由专家用户通过插件命名空间显式调用，用于修订、恢复、调试或单阶段介入。Skill 不应该自行决定跨阶段推进；跨阶段推进由 workflow 根据 `state.json`、评审矩阵、批准记录和 Hook 校验结果判断。

workflow 编排链路通过最小 Skill 集成契约完成自动调用、状态识别和产物追踪。

凡是需要被本插件 workflow 自动调用、状态识别和产物追踪的 Skill，必须在自身说明中声明最小集成契约：

- 可调用入口或 Skill 名称。
- 必要入参。
- 主要产物路径或产物类型。
- 可由脚本校验的完成标准，例如必要文件、必要章节、JSON schema、索引登记或状态前置条件。

该契约只服务于 workflow 集成，不替代 Skill 自身能力说明；未进入本插件 workflow 编排链路的 Skill 不强制要求声明该契约。MVP 不引入 Skill 类型枚举、独立 contract 文件、通用 Skill 标准或 Skill return protocol。

在本插件 workflow 链路内，Skill 默认由 workflow 根据持久化状态确定下一步应使用的 Agent/Skill，并在 Claude Code 会话中引导执行；也可以由专家用户通过插件命名空间显式调用，用于修订、恢复、调试或单阶段介入。Skill 不应该自行决定跨阶段推进；跨阶段推进由 workflow 根据 `state.json`、评审矩阵、批准记录和 Hook 校验结果判断。

高风险 Skill 必须内置人工确认闸口。该闸口与调用方式无关：

- 用户显式调用时，Skill 仍必须校验状态并要求人工确认。
- workflow 引导 Claude Code 会话使用对应 Agent/Skill 时，Skill 也必须在生效前展示确认摘要并等待人工确认。
...
- workflow 根据任务状态确定阶段、期望 Agent 视角和 Skill。
- Claude Code 会话按 workflow 的 next action 引导对应 Agent/Skill 执行专业任务，并判断负责产物是否达到该 Skill 定义的完成条件。
- Agent 生成或更新阶段产物，并给出必要的关键结论。
- workflow 根据调用上下文、预期产物、`state.json` 和 review matrix 判断是否调度正式 AI 评审闭环。
- 设计协同期间的自检可以帮助 Agent 修正明显问题，但不等同于正式 AI 评审，也不能直接把阶段状态推进到 `ai_review_passed`。

处理规则：

- Skill 在会话中表达 `success` 时，只能作为提示；只有预期产物存在、格式正确、索引登记完整且状态转移合法时，workflow 才能计算下一步 `nextAction`。
- Skill 在会话中表达 `needs_input` 时，只能作为提示；workflow 需要依据产物中的 open questions、missing info、未确认 assumption 或缺失证据等可检查事实，计算人工澄清或补充信息的 `nextAction`。
- Skill 在会话中表达 `failed` 时，只能作为提示；workflow 根据预期产物缺失、脚本校验失败或状态前置条件不满足来判断失败，不推进状态。

证据、假设、开放问题和下游处理继续由设计产物、evidence 索引、decision 记录、review matrix 和 Hook 一致性检查处理。MVP 不新增 `ready_for_review` 消息结构。
...
分别处理单个设计阶段，支持 strict 模式和局部修订。阶段 Skill 只允许读写对应阶段产物，不能顺带重写其他阶段产物。

Claude Code plugin Skill 入口名不使用空格子命令。修订动作通过 Skill 参数表达，由 `SKILL.md` 使用 `$ARGUMENTS` 或位置参数解析。推荐约定：
...
`revise` 不是新的 Claude Code command，也不是入口名中的空格子命令；它只是阶段 Skill 的显式参数模式。
...
`feature-review` 是评审方法 Skill，不负责选择评审 Agent。workflow / feature resolver 根据基础评审矩阵和风险增强规则计算期望参与的评审角色；Claude Code 会话按该角色清单引导对应 Agent 使用 `feature-review` 并以自身职责视角评审目标产物。`feature-review` 只接收 `--target <artifact>` 参数，不接收 `--reviewer` 参数。
...
`feature-implement` 首次从 `implementation_planned` 进入代码修改前，必须展示实现摘要、目标 repo、预计修改范围、验证命令和主要风险，并等待人工明确确认。确认事实写入 implementation log，不新增独立 approval 文件或状态。

人工确认后，`feature-implement` 将任务状态更新为 `implementing`。该状态表示任务处于实现阶段，允许多轮编码、修复和补测试，不表示单次命令正在运行。

任务已处于 `implementing` 时，修复、补测试和验证失败回修可以继续执行，不重复要求启动确认。

MVP 不做代码修改范围硬阻断。`feature-implement` 在认为代码落地完成前，必须基于 `git diff` / `git status` 或等价方式生成 diff summary，至少包含：
...
如果存在明显范围偏差，`feature-implement` 不自动回退、不自动阻断已发生修改；它必须提示人工确认，并把偏差说明和确认结果写入 implementation log。
...
- AI 评审-修订闭环可执行。
- 批准记录可生成。
- workflow/Skill/Hook 可阻止未批准任务通过插件流程进入代码落地。
- 开发执行计划可生成。
- 代码落地阶段能读取 repo 绑定。
- 转测包可生成。
