# Feature Clarify 评审循环 — 实施计划

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 修改 `skills/feature-clarify/SKILL.md`：阶段3增加 clarification-log 持续写入，阶段6改为按 spec 模板写入，新增阶段7评审循环，阶段8精简为最终确认+状态推进。

**Architecture:** 单文件修改。`skills/feature-clarify/requirement.md` 和 `skills/feature-clarify/checklist.md` 为静态模板，已存在无需改动。

**Tech Stack:** Markdown

## Global Constraints

- 状态推进唯一入口不变：`feature-workflow.js set-task-status clarified`
- knowledge-query 子 Agent 派发规则不变
- requirement.md 和 checklist.md 为 skill 目录静态模板，不被运行时修改
- `designRevisionLimit` 复用现有机制（默认 25）
- skill-contracts 测试必须通过

---

### Task 1: 重写 SKILL.md（新增评审循环）
...
Agent 内部判断哪些属于需求、哪些属于技术约束、哪些应延后到设计阶段。
...
git commit -m "feat(feature-clarify): add review loop with checklist subagent

- 阶段3c: add continuous clarification-log.md writing after each answer
- 阶段6: replace free-form writing with spec template (11 sections)
- 阶段7(new): review loop — subagent checks requirement.md against checklist
- 阶段8: condense to final confirm + state advance
- Add review subagent instruction prompt
- Reuse designRevisionLimit for max review rounds
- Add 3d return path for review loop re-entry"
```
