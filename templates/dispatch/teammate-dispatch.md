...
- 禁止绕过发布流程直接修改正式 artifacts；设计内容先写入当前 Draft。
...
3. 遵循 Lead 传入的 decisionPolicy={{decisionPolicy}}：需要 Lead 决策时记录待确认问题并通知 Lead；允许自主判断时记录 assumption 并继续。不要读取或判断编排配置。
4. vague 需求:按维度拆解(用户角色/核心实体/生命周期/范围/非功能),逐项暴露需要确认的空白。
5. 需要 Lead 决策的问题未完成时暂停定稿。
{{/design}}
