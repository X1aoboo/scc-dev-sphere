...
- **输出:** `artifacts/implementation-design.md`、repository evidence、`decisions/implementation-design-decisions.json`、交接契约
- **完成标准:** 见文末
...
**允许写入：** `artifacts/implementation-design.md`、`decisions/implementation-design-decisions.json`、`evidence/repository/` 与 registry。
**禁止写入：** 其他阶段产物、`state.json`、`reviews/`、`approvals/`、**源代码**（本阶段只设计，不写代码）。评审任务由 `feature-review` Skill 负责写入角色评审快照。
