...
可通过 `scripts/knowledge-query.js read-config <workspaceRoot>` 获取当前生效的数据源配置（脚本自动处理两层 fallback）。

## 查到后的处理

**必须使用脚本，禁止手动操作 evidence 文件**：

1. 将查询结果写入临时文件（如 `/tmp/kq-result-<timestamp>.md`）
2. 调用 `node scripts/knowledge-query.js register-evidence <workspaceRoot> "<主题描述>" <临时文件路径>`
