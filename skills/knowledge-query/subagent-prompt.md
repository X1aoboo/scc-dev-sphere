...
当前生效的数据源配置由主流程注入，无需自行读取。如需获取，可通过 `node scripts/knowledge-query.js read-config ${CLAUDE_PROJECT_DIR}` 获取。
...
2. 通过 stdin 传入脚本：`echo "<Content Summary>" | node scripts/knowledge-query.js register-evidence ${CLAUDE_PROJECT_DIR} "<主题描述>" <sourceType> "<query>"`
   - sourceType: 按实际查询来源填写（skill / local / repo / mcp / web）
