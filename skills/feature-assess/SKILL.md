...
直接在对话中以自然语言等待用户回复，**不要使用 AskUserQuestion 工具**（该工具仅支持选择题，不支持开放式输入）。

用户可以选择确认推荐模式，也可以指定其他模式。

如果选择 `collaborative-design`，继续以自然语言提问：「哪些设计阶段需要人工门禁确认？可选：businessDesign、solutionDesign、implementationDesign、testDesign。输入逗号分隔列表或 'none'。」
