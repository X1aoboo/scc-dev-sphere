...
导入并执行 `validateClarification(state.clarification)`。如果 `state.status !== 'clarified'` **或** validation 不完整，**MUST NOT assess**。停止评估，不写入任何评估结果，显示缺失项，并路由用户回 `/scc-dev-sphere:feature-clarify`；不得仅因 `clarified` 状态字符串而绕过验证。只有状态和 validation 都通过才可以进入步骤2。
