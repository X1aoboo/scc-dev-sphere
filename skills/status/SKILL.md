...
同时按当前 artifact 的 `version` 读取 `reviews/<artifact>/<role>.json`，展示评审是否已授权、已完成或存在旧版本快照；角色快照未全部完成时，不把 matrix 的旧状态展示为当前版本已通过。
...
- 风险候选项: {count}
- 当前版本 Reviewer: {completed}/{required}（按 artifactVersion）
