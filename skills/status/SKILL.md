...
2. 运行 `"${CLAUDE_PLUGIN_ROOT}/bin/devsphere" design inspect-workspace --task-path "<taskPath>"`，展示外层要求的设计类型、已有 Baseline 和从 Work/Draft 推断的当前设计活动。
...
5. 运行 `"${CLAUDE_PLUGIN_ROOT}/bin/devsphere" workflow resolve-next-action`，只展示顶层下一步建议。
