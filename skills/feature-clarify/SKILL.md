...
根据原始需求构造查询意图（涉及的业务规则、系统/模块、接口、数据、权限、性能、部署等）。**MUST dispatch a one-shot `knowledge-query` subagent**，其 prompt 必须要求返回结构化 `EV/gap` 结果。**MUST wait for the structured EV/gap result**，将每项 gap 通过 `recordEvidenceGap` 写入 clarification；事实只能作为待用户确认的候选依据。
...
使用 AskUserQuestion 的 `confirm_gate` 请求最终确认。只有用户确认最终内容后，保存 clarification 与 requirement markdown，并执行：
