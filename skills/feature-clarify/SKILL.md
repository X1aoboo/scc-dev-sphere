...
在主会话完成需求澄清。澄清结论追加到 `inputs/requirement.md`；是否完成按下方「完成判断原则」判断。**MUST NOT directly query the knowledge base in the main session** —— 知识查询只能派发一次性子 Agent 执行（见下）。每次派发都是新的 Task：**MUST NOT reuse agent IDs**，**MUST NOT use teammate**，不跨轮恢复。
...
- **写入:** 追加到 `inputs/requirement.md` 的澄清区块（**永不覆盖原始需求**）。
