...
分析原始需求（不写入文件），识别：明确提出了什么、Agent 的关键推断、当前缺口。
...
**`knowledge-query` Skill 是查询既有事实/系统上下文的唯一入口。**

**严禁 以Bash/Read/Glob/Grep 等方式直接探索当前工作区或仓库作为替代。 **

执行规则：

- **知识库（knowledge-query 配置的数据源）是唯一事实源。** 必须且只能调用 `knowledge-query` Skill。
- 设计阶段所在项目与真实实现项目不一定同处一仓，当前任务工作区是设计产物载体，不能擅自认为为事实源。
- **`knowledge-query` 返回 gap（未找到）绝不等于「系统未实现 / 功能不存在」。** gap 仅表示该知识不在当前配置的数据源中。不得据此断定任何功能缺失或未实现。
